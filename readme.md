...
The benchmark focuses on testing **threshold-based labeling functions** that operate on numerical features. Each LF has:
- A subset of input features it examines (typically 4-10 features per LF)
- Threshold values for each feature  
- Comparison operators (>, ≤) combined with AND logic
- A label to assign (binary: 1/0) when all conditions are met / when some conditions violated
...
Run on GPU: NVIDIA GeForce RTX 2070
